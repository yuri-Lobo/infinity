...
import { SortDirection } from "./servico-sortable.directive";
import {
  SearchResult,
  ServicoGeral,
} from "src/app/core/models/servico/servicoGeral.model";
...
/**
 * Sort the table data
 * @param tabless ServicoGeral field value
...
/**
 * ServicoGeral Data Match with Search input
 * @param tables ServicoGeral field value fetch
 * @param term Search the value
 */
function matches(tables: ServicoGeral, term: string, pipe: PipeTransform) {
  return (
    tables.codigo.toLowerCase().includes(term) ||
    tables.descricao.toLowerCase().includes(term) ||
    tables.unidade.toLowerCase().includes(term)
  );
}
...
export class CadastroServicoService {
  ...
  // tslint:disable-next-line: variable-name
  private _tables$ = new BehaviorSubject<ServicoGeral[]>([]);
  // tslint:disable-next-line: variable-name
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let tables = sort(dadosCadstroServico, sortColumn, sortDirection);
