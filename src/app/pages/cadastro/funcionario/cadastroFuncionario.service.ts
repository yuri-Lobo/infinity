...
import { SearchResult } from "src/app/core/models/equipe.models";
import { SortDirection } from "./funcionario-sortable.directive";
...
/**
 * Sort the table data
 * @param tabless Funcionario field value
...
/**
 * Funcionario Data Match with Search input
 * @param tables Funcionario field value fetch
 * @param term Search the value
 */
function matches(tables: Funcionario, term: string, pipe: PipeTransform) {
  return (
    tables.id.toString().toLowerCase().includes(term) ||
    tables.cpf.toLowerCase().includes(term) ||
    tables.funcao.toString().toLowerCase().includes(term) ||
    tables.nome.toLowerCase().includes(term)
  );
}
...
export class CadastroFuncionarioService {
  ...
  // tslint:disable-next-line: variable-name
  private _tables$ = new BehaviorSubject<Funcionario[]>([]);
  // tslint:disable-next-line: variable-name
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let tables = sort(dadosCadastroFuncionario, sortColumn, sortDirection);
